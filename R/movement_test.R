...
ad_cell_no<-ad_cell_no[1:10]
juve_cell_no<-juve_cell_no[1:10]
all_cell_no<-c(ad_cell_no,juve_cell_no)
...
dmatrix <- matrix(data = 0, nrow = 20, ncol=20)#length(all_cell_no), ncol = length(all_cell_no) )
...
juve_df$dist[juve_df$from %in% ad_cell_no]<-NA
juve_df$dist[juve_df$to %in% ad_cell_no]<-NA
#juve_juve_df<-dframe[which(dframe$from %in% juve_cell_no), ]
#juve_juve_df<-juve_adult_df[which(juve_adult_df$to %in% juve_cell_no), ]

write.csv(juve_df,paste0(boxdir,"/",runname,"/juve_distance.csv"))


adult_df<-dframe
adult_df$dist[adult_df$from %in% juve_cell_no]<-NA
adult_df$dist[adult_df$to %in% juve_cell_no]<-NA
...
juve_adult_df<-dframe
juve_adult_df$dist[juve_adult_df$from %in% ad_cell_no]<-NA
juve_adult_df$dist[juve_adult_df$to %in% juve_cell_no]<-NA
#adult_juve_df<-dframe[which(dframe$from %in% ad_cell_no), ]
#adult_juve_df<-adult_juve_df[which(adult_juve_df$to %in% juve_cell_no), ]

write.csv(juve_adult_df,paste0(boxdir,"/",runname,"/adult_juve_distance.csv"))
